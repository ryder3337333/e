...
export default function LoadoutScreen() {
  ...
  const load = useCallback(async () => {
    try { setLoadouts(await api<Loadout[]>('/loadouts')); } catch (e) { console.warn(e); }
  }, []);
  ...
  const save = async () => {
    if (!name.trim()) { Alert.alert('Name required', 'Enter a build name.'); return; }
    try {
      await api('/loadouts', { method: 'POST', body: { name: name.trim(), enchantments: ench, armor, notes } });
  ...
  const remove = async (id: string) => {
    await api(`/loadouts/${id}`, { method: 'DELETE' });
    await load();
  };
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <ScrollView contentContainerStyle={styles.content}>
            ...
            <View style={styles.card}>
              ...
              <TextInput testID="loadout-name-input" value={name} onChangeText={setName}
                placeholder="BUILD NAME" placeholderTextColor={theme.colors.textSecondary} style={styles.input} />
              <Text style={styles.subHeader}>ENCHANTMENTS</Text>
              <View style={styles.chips}>
                {ENCHANT_OPTIONS.map((e) => (
                  <TouchableOpacity key={e} testID={`ench-${e}`} onPress={() => toggle(ench, setEnch, e)}
              ...
              <View style={styles.chips}>
                {ARMOR_OPTIONS.map((a) => (
                  <TouchableOpacity key={a} testID={`armor-${a}`} onPress={() => toggle(armor, setArmor, a)}
              ...
              <TextInput testID="loadout-notes-input" value={notes} onChangeText={setNotes}
                placeholder="NOTES (combos, server, etc.)" placeholderTextColor={theme.colors.textSecondary}
                style={[styles.input, { marginTop: 12, height: 70 }]} multiline />
            ...
            {loadouts.length === 0 && (<Text style={styles.empty}>No builds yet — forge your first one above.</Text>)}
            {loadouts.map((l) => (
...
const styles = StyleSheet.create({
  ...
  cardHeader: { fontFamily: theme.font, fontSize: 14, color: theme.colors.gold, fontWeight: 'bold', textTransform: 'uppercase', marginBottom: 8, letterSpacing: 1 },
  subHeader: { fontFamily: theme.font, fontSize: 12, color: theme.colors.text, marginTop: 12, marginBottom: 6, fontWeight: 'bold' },
  input: { backgroundColor: theme.colors.obsidian, borderColor: theme.colors.borderDark, borderWidth: 4, color: theme.colors.text, padding: 12, fontSize: 14, fontFamily: theme.font },
  ...
  saveBtn: { backgroundColor: theme.colors.emerald, borderColor: theme.colors.emeraldDark, borderWidth: 4, borderBottomWidth: 8, paddingVertical: 12, alignItems: 'center', marginTop: 12 },
  saveTxt: { fontFamily: theme.font, fontSize: 14, fontWeight: 'bold', color: '#fff', textTransform: 'uppercase', textShadowColor: '#000', textShadowOffset: { width: 1, height: 1 } },
  sectionHeader: { fontFamily: theme.font, fontSize: 14, color: theme.colors.gold, fontWeight: 'bold', textTransform: 'uppercase', marginBottom: 8, letterSpacing: 1 },
  empty: { fontFamily: theme.font, color: theme.colors.textSecondary, textAlign: 'center', padding: 16 },
  buildCard: { backgroundColor: theme.colors.dirtDark, borderColor: theme.colors.borderDark, borderWidth: 4, padding: theme.spacing.md, marginBottom: theme.spacing.sm },
  cardHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  buildTitle: { fontFamily: theme.font, fontSize: 16, color: theme.colors.gold, fontWeight: 'bold', textTransform: 'uppercase' },
  ...
  notes: { fontFamily: theme.font, fontSize: 12, color: theme.colors.textSecondary, marginTop: 8, fontStyle: 'italic' },
});
